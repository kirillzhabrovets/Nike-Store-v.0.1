...
const Item = ({ ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
  ...
  return (
    <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists ? 'justify-item-start':'justify-item-center'} rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
        
            <div className={`grid items-center ${ifExists ? 'justify-item-start':'justify-item-center'}`}>
                ...
                <div className={`flex items-center ${ifExists ? " absolute top-5 right-5":'justify-center'}`}>
                    <img 
                        src={img}
                        alt={`img/item-img/${id}`}
